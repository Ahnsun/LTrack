...
class Detector(object):
    def __init__(self, args, model=None, seq_num=2):
        ...
        img_list = os.listdir(os.path.join(self.args.mot_path, 'DanceTrack/val', self.seq_num, 'img1'))
        img_list = [os.path.join(self.args.mot_path, 'DanceTrack/val', self.seq_num, 'img1', _) for _ in img_list if
        ...
        self.predict_path = os.path.join(self.args.output_dir, 'preds_e184_09_no_nms', self.seq_num)
    ...
    @staticmethod
    def filter_dt_by_nms(dt_instances: Instances, iou_threshold: float) -> Instances:
        boxes = dt_instances.boxes
        scores = dt_instances.scores
        keep = torchvision.ops.nms(boxes=boxes, scores=scores, iou_threshold=iou_threshold)
        return dt_instances[keep]
    ...
    def eval_seq(self):
        data_root = os.path.join(self.args.mot_path, 'DanceTrack/val')
    ...
    def detect(self, prob_threshold=0.9, area_threshold=200, vis=False):
        total_dts = 0
        ...
        for i in tqdm(range(0, self.img_len)):
            ...
            # dt_instances = self.filter_dt_by_nms(dt_instances, 0.85)
