...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, drop_path_rate=0.):
        ...
        self.resblocks = nn.Sequential(*[ResidualAttentionBlock(width, heads, self.attn_mask, dpr[i]) for i in range(layers)])
...
class CLIPTextContextEncoder(nn.Module):
    ...
    #     x_text = self.token_embedding(text)  # n_class, n_text, C
    #     K, N1, C = x_text.shape
    ...
    #     x_text = self.token_embedding(text)  # n_class, n_text, C
    #     _, N1, C = x_text.shape
    #     B, N2, K, C = context.shape

    #     eos_indx = (text.argmax(dim=-1) + N2).expand(K)
    #     eos_indx = eos_indx.reshape(1, K).expand(B, K).reshape(-1)

    #     x_text = x_text.reshape(1, 1, N1, C).expand(B, K, N1, C)
    ...
    # only text
    def forward(self, text, context, mode):
        if mode == 'onlytext':
            x_text = self.token_embedding(text)  # n_class, n_text, C
            K, N1, C = x_text.shape

            eos_indx = text.argmax(dim=-1)
            eos_indx = eos_indx.reshape(1, K).expand(1, K).reshape(-1)
            ...
            x = self.ln_final(x)
            x = x[torch.arange(x.shape[0]), eos_indx] @ self.text_projection
            x = x.reshape(1, K, self.embed_dim)
        ...
        elif mode == 'query_concat_text':
            track_text, text = text
            ...
            # 48 track description pool
            pool_text = self.token_embedding(track_text)
            K3, N3, C3 = pool_text.shape

            eos_indx3 = track_text.argmax(dim=-1)
            eos_indx3 = eos_indx3.reshape(1, K3).expand(1, K3).reshape(-1)

            x = torch.cat([x, pool_text], dim=0)
            x = x + self.positional_embedding

            mask = torch.empty(K+K3, K+K3)
            mask.fill_(float("-inf"))
            mask.triu_(1)  # zero out the lower diagonal
            mask[K:,:K] = float("-inf")

            for module in self.transformer.resblocks:
                module.attn_mask = mask
            # x = x.permute(1, 0, 2)  # NLD -> LND       N = K + K3
            ...
            x_t = x_t[torch.arange(x_t.shape[0]), eos_indx] @ self.text_projection
            x_t = x_t.reshape(B, K, self.embed_dim)

            x_p = x_p[torch.arange(x_p.shape[0]), eos_indx3] @ self.text_projection
            x_p = x_p.reshape(B, K3, self.embed_dim)
...
class ContextDecoder(nn.Module):
    def __init__(self,
        ...
        self.out_proj = nn.Sequential(
            nn.LayerNorm(transformer_width),
            nn.Linear(transformer_width, outdim)
        )
    ...
    def forward(self, track_query, visual):
        B, N, C = visual.shape
        visual = self.memory_proj(visual)
        # x = self.text_proj(text)
        x = track_query if self.memory else track_query.expand(B, -1, -1)
        if self.memory:
            visual = visual.expand(track_query.shape[0], -1, -1)
...
class Pre_ContextDecoder(nn.Module):
    def __init__(self, context_length=16,
        ...
        self.query = nn.Parameter(torch.randn(1, context_length, transformer_width))
