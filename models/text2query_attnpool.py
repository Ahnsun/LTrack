...
class ClipMatcher(SetCriterion):
    ...
    def match_for_single_frame(self, outputs: dict):
        ...
        full_track_idxes = torch.arange(len(track_instances), dtype=torch.long).to(pred_logits_i.device)    # initial 0~299
...
class AttentionPool2d(nn.Module):
    def __init__(self, spacial_dim: list, embed_dim: int, num_heads: int, output_dim: int = None):
        super().__init__()
    ...
    def forward(self, x):
        ...
        spatial_pos = F.interpolate(self.positional_embedding[1:,].reshape(1, self.spacial_dim[0], self.spacial_dim[1], self.embed_dim).permute(0, 3, 1, 2), size=(H, W), mode='bilinear')
        # spatial_pos = self.positional_embedding[1:].reshape(self.spacial_dim[0], self.spacial_dim[1], self.embed_dim)[:H, :W]
...
class MOTR(nn.Module):
    def __init__(self, backbone, transformer, num_classes, num_queries, num_feature_levels, criterion, track_embed,
        ...
        self.attnpool = AttentionPool2d(spacial_dim = [850, 608], embed_dim = 1024, num_heads = 4, output_dim = 512)
    ...
    def _forward_single_image(self, samples, track_instances: Instances):
        ...
        global_feat, context = self.attnpool(src)
        assert mask is not None
        # obtain text embedding
        B, C, H, W = context.shape
        visual_context = torch.cat([global_feat.reshape(B, C, 1), context.reshape(B, C, H*W)], dim=2).permute(0, 2, 1)  # B, N, C
        # visual_context = context.reshape(B, C, H*W).permute(0, 2, 1)  # B, N, C
